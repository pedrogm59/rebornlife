<?php
include '../init.php';
$vehicles = $DB->prepare("SELECT * FROM vehicles_med ORDER BY id DESC");
$vehicles->execute();
...
if (isset($_GET) and isset($_GET['vehicule']) and isset($_GET['sendHome'])) //Renvoi véhicule au garage
{
    ...
    $name = $player->name;
    $DB->exec("UPDATE vehicles_med SET active=0 WHERE id=$idVehicule;");
    $DB->exec("INSERT INTO logs_panel (username, texte) VALUES ('$paneluser','Véhicule SP ($idVehicule) renvoyé au garage.')");
...
else if (isset($_POST) and isset($_GET['addVehicule']) and Auth::isAdmin()) //Ajout véhicule
{
    ...
    try {
        ...
        $res = $DB->prepare('SELECT id FROM vehicles_med WHERE plate = :plaque') or die(print_r($DB->errorInfo()));
        ...
        $DB->exec("INSERT INTO vehicles_med (classname, type, pid, alive, active, plate, gear, position, pos_save, pos_check, fuel, damage, isInHome, lockveh, locktime, insert_time) 
                                      VALUES ('$classname', '$type', '', 1, 0, '$plaque', '[]', '[]', '[]', '[]', 1, '[]', 0, 0, 0, '$insertTime');");
        $DB->exec("INSERT INTO logs_panel (username, texte) VALUES ('$paneluser','Véhicule SP ($classname) ajouté au garage. [$raison]')");
        $_SESSION['alert'] = ['type' => 'success', 'message' => 'Véhicule ajouté au garage SP.'];
        header('Location: /SapeursPompiers/Garage.php');
        exit();
...
else if (isset($_POST) and isset($_GET['editVehicule']) and Auth::isModo()) //Modification véhicule
{
    ...
    $res = $DB->prepare('SELECT classname FROM vehicles_med WHERE id=:id') or die(print_r($DB->errorInfo()));
    ...
    try {
        ...
        $res = $DB->prepare('SELECT id FROM vehicles_med WHERE plate = :plaque AND id != :id') or die(print_r($DB->errorInfo()));
        $res->bindParam(':plaque', $plaque);
        $res->bindParam(':id', $idVehicule);
        $res->execute();
        if (isset($res->fetch(PDO::FETCH_ASSOC)['id'])) {
        ...
        if (Auth::isAdmin())
            $DB->exec("UPDATE vehicles_med SET classname='$classname', type='$type', plate = '$plaque', peinture = '$peinture', fuel='$fuel' WHERE id = '$idVehicule'");
        else
            $DB->exec("UPDATE vehicles_med SET fuel='$fuel' WHERE id = '$idVehicule'");

        $DB->exec("INSERT INTO logs_panel (username, texte) VALUES ('$paneluser','Véhicule SP ($classname) modifié. [$raison]')");
...
?>
    <header class="row m-3">
        <div class="col-md aliSP-content-center">
            <h2 class="text-center display-4">Sapeurs-Pompiers - Garage <span class="badge badge-dark"><?= $vehicles->rowCount() ?></span></h2>
        </div>
    </header>
                ...
                <td><?php if (Auth::isStaff()) { ?>
                        <div class="btn-group btn-group-sm">
                            <?php if ($vehicle->active == 0) { ?>
                                <a onclick="return confirm('Supprimer le véhicule :');" href="/Admin/deleteVehicule?id=<?= $vehicle->id ?>&garage=SapeursPompiers"
                                   class="btn btn-sm btn-danger">
                                    <span class="fa fa-trash"></span></a>
                            <?php } else { ?>
                                <a href="/SapeursPompiers/Garage.php?vehicule=<?= $vehicle->id ?>&sendHome" class="btn btn-primary"
...
<?php if (Auth::isAdmin()) { ?>
    <div class="modal fade" id="addVehiculeModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <form method="post" action="/SapeursPompiers/Garage.php?addVehicule">
...
<?php } ?>
    ...
    <script>
        function editVehicule(id, type, classname, plaque, peinture, fuel) {
            $('#editVehiculeForm').attr('action', "/SapeursPompiers/Garage.php?editVehicule=" + id);
