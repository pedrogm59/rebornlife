<?php
include '../init.php';
$search = $DB->query("SELECT DISTINCT * FROM players WHERE mediclevel != '0' ORDER BY mediclevel DESC, name ASC");
include '../templates/top.php';
?>
    <header class="row m-3">
        <div class="col-md align-content-center">
            <h2 class="text-center display-4">Sapeurs-Pompiers <span class="badge badge-dark"><?= $search->rowCount() ?></span></h2>
        </div>
    </header>
    <div class="row">
        <div class="col">
            <table class="table table-responsive-sm table-hover table-bordered bg-white">
                ...
                <?php while ($row = $search->fetch(PDO::FETCH_OBJ)) {
                    ...
                    $res6 = $DB->prepare('SELECT * FROM medservice WHERE name = :playername ORDER BY insert_time ASC') or die(print_r($DB->errorInfo()));
