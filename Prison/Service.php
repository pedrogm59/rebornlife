...
$search = $DB->prepare("SELECT DISTINCT * FROM logs_alf JOIN players ON logs_alf.name = players.name WHERE players.name LIKE :search_value AND type='SERVICE' AND (text LIKE '%service Penit.' OR text LIKE '%service AP.') ORDER BY id DESC LIMIT 0, 200");
...
?>
    <header class="row m-3">
        <div class="col-md align-content-center">
            <h2 class="text-center display-4">Administration Pénitentiaire - Service</h2>
        </div>
    </header>
    <form method="get" id="searchService">
        <div class="input-group mb-2">
            <input name="q" id="q" title="Recherche PDS et FDS" placeholder="Recherche" class="form-control" type="search" autocomplete="off" value="<?= $search_value ?>"/>
