...
include '../init.php';
if (!Auth::isAdmin()) {
    $_SESSION['alert'] = ['type' => 'warning', 'message' => "Vous n'avez pas accès aux logs Admin IG."];
    header('Location: /');
    exit();
}
...
if ($page < 1) $page = 1;
$search = $DB->prepare("SELECT DISTINCT * FROM logs_alf WHERE (name LIKE :search_value OR uid LIKE :search_value) AND type = 'ADMIN' ORDER BY insert_time DESC LIMIT 200 OFFSET ".($page - 1) * 200 .";");
...
?>
    <header class="row m-3">
        <div class="col-md align-content-center">
            <h2 class="text-center display-4">Logs - Admins IG</h2>
        </div>
    </header>
    <div class="row">
        <div class="col">
        ...
        <nav class="col-auto mx-auto">
            <ul class="pagination">
                <li class="page-item<?php if ($page <= 1) { ?> disabled<?php } ?>">
                    <a class="page-link" href="/Admin/LogsAdmin.php?page=<?= $page - 1 ?>&q=<?= $search_value ?>"><< Précédent</a>
                </li>
                <li class="page-item disabled">
                    <a class="page-link" href="/#"><?= $page ?></a>
                </li>
                <li class="page-item<?php if ($search->rowCount() == 0 || $search->rowCount() < 200) { ?> disabled<?php } ?>">
                    <a class="page-link" href="/Admin/LogsAdmin.php?page=<?= $page + 1 ?>&q=<?= $search_value ?>">Suivant >></a>
                </li>
            </ul>
        </nav>
    </div>
